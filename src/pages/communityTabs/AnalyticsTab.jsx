...
export default function AnalyticsTab({
  ...
  const maxMonthlyAbs = Math.max(1, ...seasonal.monthly.map((m) => Math.abs(m.netIncome)));
    ...
    <div className="section">
      ...
      <div className="analytics-grid">
        <div className="season-card">
          ...
          <div className="season-monthly-chart">
            {seasonal.monthly.map((m, i) => (
              <div className="season-monthly-col" key={i}>
                <div
                  className={m.isRainy ? 'season-monthly-bar rainy-m' : 'season-monthly-bar dry-m'}
                  style={{ height: `${Math.max((Math.abs(m.netIncome) / maxMonthlyAbs) * 100, 4)}%` }}
                />
                <div className="season-monthly-label">{m.label}</div>
              </div>
            ))}
          </div>
        </div>
